...
const NFTState = {
  ...
  getInitialState() {
    return {
      ...
      lastUpdated: null,
      walletAddress: null,
      ftsoProviders: [], // Added for FTSO provider storage
      lastFTSOFetch: null // Added to track last FTSO fetch time
    };
  },
  ...
  saveToStorage() {
    try {
      ...
      const serializedData = {
        ...
        lastUpdated: Date.now(),
        ftsoProviders: this.data.ftsoProviders,
        lastFTSOFetch: this.data.lastFTSOFetch
      };
  ...
  loadFromStorage() {
    try {
      ...
      return {
        ...
        ftsoProviders: parsed.ftsoProviders || [],
        lastFTSOFetch: parsed.lastFTSOFetch || null
      };
    } catch (error) {
  ...
  setFTSOProviders(providers) {
    try {
      this.data.ftsoProviders = providers;
      this.saveToStorage();
      console.log(`Saved ${providers.length} FTSO providers to state`);
    } catch (error) {
      console.error("Error setting FTSO providers:", error);
    }
  },

  getFTSOProviders() {
    return this.data?.ftsoProviders || [];
  }
};
...
async function toggleTokenDisplay(contractAddr, provider, scanningProvider) {
  ...
  if (isHidden) {
    ...
    await displayTokenPage(contractAddr, currentPage, provider, scanningProvider);
  } else {
...
function updateCollectionUI(collection, collectionElement) {
  const balanceText = collectionElement.querySelector('p:nth-child(3)');
...
async function displayTokenPage(contractAddr, page, provider, scanningProvider) {
  ...
  try {
    const contract = new ethersLib.Contract(collection.distributorAddress, ROYALTY_ABI, scanningProvider);
...
async function processBatch(tokenIds, contract, scanningProvider) {
  try {
    ...
    const erc721Contract = new ethersLib.Contract(erc721ContractAddress, NFT_ABI, scanningProvider);
    for (const tokenId of tokenIds) {
...
async function handleClaim(tokenIds, container, contractAddr, provider, scanningProvider) {
  ...
  try {
    ...
    for (let i = 0; i < tokenIds.length; i += BATCH_SIZE) {
      ...
      const batchSuccess = await processBatch(batch, contract, scanningProvider);
...
async function showAddCollectionForm(contractAddress, provider) {
  ...
  const container = document.querySelector('.container.mx-auto.px-4.py-6.max-w-5xl') || document.body;
...
async function delegateToFTSO(provider, contractAddress, ftsoProviderAddress) {
  ...
  try {
    if (!ethersLib.utils.isAddress(ftsoProviderAddress)) {
      throw new Error("Invalid FTSO provider address");
    }
    ...
    const network = await provider.getNetwork();
    const chainId = network.chainId;
    const wnatAddress = CHAIN_CONFIGS[chainId].paymentTokenAddress;
    const wnatContract = new ethersLib.Contract(
      wnatAddress,
      ['function balanceOf(address) view returns (uint256)'],
      provider
    );
    const contractBalance = await wnatContract.balanceOf(contractAddress);
    if (contractBalance.eq(0)) {
      throw new Error("RoyaltyDistributor contract has no WNAT to delegate");
    }

    const tx = await royaltyContract.setFtsoProvider(ftsoProviderAddress, { gasLimit: 300000 });
    console.log(`Transaction submitted: ${tx.hash}`);
    await tx.wait();
    console.log(`Delegated 100% WNAT to FTSO provider ${ftsoProviderAddress} for contract ${contractAddress}`);
    showNotification(`Delegated 100% to ${ftsoProviderAddress}`, "success");
  } catch (error) {
    console.error("Error delegating to FTSO:", error);
    showNotification(`Failed to delegate: ${error.message || 'Transaction reverted without reason'}`, "error");
    throw error;
  }
}

// FTSO Provider Fetching
async function fetchFTSOProviders(chainId) {
  const apiUrls = {
    14: 'https://flare-systems-explorer-backend.aflabs.org/api/v0/entity',
    19: 'https://songbird-systems-explorer.flare.network/api/v0/entity'
  };

  const apiUrl = apiUrls[chainId];
  if (!apiUrl) {
    console.error(`No FTSO API endpoint defined for chain ${chainId}`);
  ...
  while (retries < MAX_RETRIES) {
    try {
      const proxyUrl = `/api/proxy?url=${encodeURIComponent(`${apiUrl}?limit=100`)}`;
      console.log(`Fetching top 100 FTSO providers (attempt ${retries + 1}): ${proxyUrl}`);
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch FTSO providers: ${response.status} - ${errorText}`);
      }
      ...
      if (!Array.isArray(data.results)) {
        console.error(`Invalid API response format: results=${JSON.stringify(data.results)}`);
        throw new Error('Invalid API response format');
      }

      const providers = data.results
        .filter(provider => {
          const delegationAddress = provider.denormalizedentity?.delegation_address;
          const hasDelegationAddress = delegationAddress && ethersLib.utils.isAddress(delegationAddress);
          const isValid = provider.listed && hasDelegationAddress;
          console.log(`Provider ID ${provider.id} (${provider.display_name || 'Unknown'}): ` +
            `primary=${provider.providersuccessrate?.primary}, listed=${provider.listed}, ` +
            `identity_address=${provider.identity_address}, ` +
            `delegation_address=${delegationAddress}, eligible_for_reward=${provider.entityminimalconditions?.eligible_for_reward}, included=${isValid}`);
          if (!hasDelegationAddress) {
            console.warn(`Excluding provider ${provider.id}: Invalid or missing delegation_address`);
          }
          if (!provider.listed) {
            console.warn(`Excluding provider ${provider.id}: Not listed`);
          }
          return isValid;
        })
        .map(provider => ({
          id: provider.id,
          displayName: provider.display_name || `Provider ${provider.id}`,
          delegationAddress: provider.denormalizedentity.delegation_address.toLowerCase(),
          identityAddress: provider.identity_address ? provider.identity_address.toLowerCase() : null,
          logoUrl: provider.logo_url || '',
          successRate: provider.providersuccessrate ? {
            primary: provider.providersuccessrate.primary / 100,
            secondary: provider.providersuccessrate.secondary / 100,
            availability: provider.providersuccessrate.availability / 100
          } : { primary: 0, secondary: 0, availability: 0 },
          rewardRate: provider.rewards ? provider.rewards.reward_rate_wnat : 0,
          eligibleForReward: provider.entityminimalconditions?.eligible_for_reward || false
        }));

      console.log(`Fetched ${providers.length} FTSO providers for chain ${chainId}`);
      return providers;
    } catch (error) {
      retries++;
      console.error(`Fetch attempt ${retries} failed for chain ${chainId}:`, error.message);
      if (retries >= MAX_RETRIES) {
        console.error('Max retries reached for fetching FTSO providers');
        showNotification(`Failed to load FTSO providers after ${MAX_RETRIES} attempts: ${error.message}`, 'error');
        return [];
      }
...
// FTSO Provider Selection Card
function showFTSOProvidersCard(providers, onSelectCallback) {
  console.log(`showFTSOProvidersCard received ${providers?.length || 0} providers`, providers);
  const existingCard = document.querySelector('.ftso-provider-card');
  if (existingCard) existingCard.remove();

  const ftsoCard = document.createElement('div');
  ftsoCard.className = 'ftso-provider-card bg-gray-800 p-6 rounded-lg mb-6';
  ftsoCard.innerHTML = `
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-lg font-bold text-white">Select FTSO Data Provider</h2>
      <button class="close-ftso-card text-gray-400 hover:text-white text-lg">×</button>
    </div>
    <div id="ftso-loading" class="text-gray-400 text-center">Loading providers...</div>
    <div id="ftso-content" class="hidden">
      <div class="max-h-96 overflow-y-auto mb-4">
        <table class="w-full text-sm text-gray-300">
          <thead>
            <tr class="bg-gray-700">
              <th class="p-2 text-left">Name</th>
              <th class="p-2 text-left">Primary Success</th>
              <th class="p-2 text-left">Reward Rate</th>
              <th class="p-2 text-left">Reward Eligible</th>
              <th class="p-2 text-left">Action</th>
            </tr>
          </thead>
          <tbody id="ftso-providers-list"></tbody>
        </table>
      </div>
      <div id="ftso-manual-entry" class="mt-4">
        <h3 class="text-sm font-medium text-gray-300 mb-2">Or Enter FTSO Provider Address Manually</h3>
        <form id="manual-ftso-form">
          <input type="text" id="manual-ftso-address" class="w-full p-2 rounded bg-gray-700 text-white mb-2" placeholder="Enter FTSO provider delegation address" />
          <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm">Delegate to Address</button>
        </form>
      </div>
    </div>
    <div id="ftso-empty" class="text-gray-400 text-center hidden">No FTSO providers available for this network.</div>
  `;

  const container = document.querySelector('.container.mx-auto.px-4.py-6.max-w-5xl') || document.body;
  const royaltyList = document.getElementById('royalty-list');
  if (royaltyList) {
    royaltyList.insertAdjacentElement('beforebegin', ftsoCard);
  } else {
    container.appendChild(ftsoCard);
  }

  ftsoCard.querySelector('.close-ftso-card').addEventListener('click', () => ftsoCard.remove());

  if (!providers || providers.length === 0) {
    ftsoCard.querySelector('#ftso-loading').classList.add('hidden');
    ftsoCard.querySelector('#ftso-empty').classList.remove('hidden');
    ftsoCard.querySelector('#ftso-content').classList.remove('hidden');
  } else {
    const sortedProviders = providers.sort((a, b) => {
      // Prioritize reward-eligible providers, then by composite score
      const aEligible = a.eligibleForReward ? 1 : 0;
      const bEligible = b.eligibleForReward ? 1 : 0;
      if (aEligible !== bEligible) return bEligible - aEligible;
      const aScore = (a.successRate?.primary || 0) * 0.7 + (a.rewardRate || 0) * 100 * 0.3;
      const bScore = (b.successRate?.primary || 0) * 0.7 + (b.rewardRate || 0) * 100 * 0.3;
      return bScore - aScore;
    });

    const providersList = ftsoCard.querySelector('#ftso-providers-list');
    sortedProviders.forEach(provider => {
      const row = document.createElement('tr');
      row.className = 'border-b border-gray-700';
      row.innerHTML = `
        <td class="p-2 flex items-center">
          ${provider.logoUrl ? `<img src="${provider.logoUrl}" alt="${provider.displayName}" class="w-6 h-6 mr-2 rounded-full">` : ''}
          <span>${provider.displayName}</span>
        </td>
        <td class="p-2">${provider.successRate ? `${provider.successRate.primary.toFixed(2)}%` : 'N/A'}</td>
        <td class="p-2">${provider.rewardRate ? `${(provider.rewardRate * 100).toFixed(2)}%` : 'N/A'}</td>
        <td class="p-2">${provider.eligibleForReward ? '<span class="text-green-500">Yes</span>' : '<span class="text-red-500">No</span>'}</td>
        <td class="p-2">
          <button class="select-provider bg-blue-500 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm transition-colors" data-address="${provider.delegationAddress}">
            Select
          </button>
        </td>
      `;
    ...
    providersList.querySelectorAll('.select-provider').forEach(button => {
      button.addEventListener('click', () => {
        const address = button.dataset.address;
        const providerData = sortedProviders.find(p => p.delegationAddress === address);
        if (providerData) {
          const manualAddressInput = ftsoCard.querySelector('#manual-ftso-address');
          manualAddressInput.value = providerData.delegationAddress;
          manualAddressInput.classList.add('border-blue-500');
          manualAddressInput.focus();
          showNotification(`Selected ${providerData.displayName}. Confirm by clicking "Delegate to Address"`, 'info');
        }
      });
    });
  }

  const manualForm = ftsoCard.querySelector('#manual-ftso-form');
  manualForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const address = ftsoCard.querySelector('#manual-ftso-address').value.trim();
    if (!ethersLib.utils.isAddress(address)) {
      showNotification('Please enter a valid Ethereum address', 'error');
      return;
    }
    const providerData = providers.find(p => p.delegationAddress === address) || {
      delegationAddress: address,
      displayName: `Manual Provider (${address.slice(0, 6)}...${address.slice(-4)})`
    };
    if (onSelectCallback) {
      onSelectCallback(providerData);
      ftsoCard.remove();
    }
  });
}
...
function showManageContractsForm(provider) {
  ...
  if (existingForm) {
    console.log('Removing existing manage contracts form');
    existingForm.remove();
  }
  ...
  const deployForm = document.getElementById('deploy-form');
  const container = document.querySelector('.container.mx-auto.px-4.py-6.max-w-5xl') || document.body;
  console.log('Deploy form:', deployForm, 'Container:', container);
  if (deployForm) {
    console.log('Inserting manageForm after deployForm');
    deployForm.insertAdjacentElement('afterend', manageForm);
  } else {
    console.log('Inserting manageForm as first child of container');
    container.insertBefore(manageForm, container.firstChild);
  }
  ...
  form.querySelector('#delegate-ftso-btn').addEventListener('click', async () => {
    ...
    const network = await provider.getNetwork();
    const chainId = network.chainId;
    let providers = NFTState.getFTSOProviders();
    const CACHE_EXPIRATION_MS = 60 * 60 * 1000;
    const lastFetched = NFTState.getData()?.lastFTSOFetch || 0;
  
    showFTSOProvidersCard([], () => {});
  
    if (providers.length === 0 || Date.now() - lastFetched > CACHE_EXPIRATION_MS) {
      console.log(`Fetching top 100 FTSO providers for chain ${chainId}`);
      providers = await fetchFTSOProviders(chainId);
      if (providers.length > 0) {
        NFTState.setFTSOProviders(providers);
        NFTState.setData({ ...NFTState.getData(), lastFTSOFetch: Date.now() });
      }
    } else {
      console.log(`Using cached FTSO providers (${providers.length})`);
    }
  
    showFTSOProvidersCard(providers, async (selectedProvider) => {
      console.log(`Selected FTSO provider: ${selectedProvider.delegationAddress}`);
      try {
        await manageContractAction(contractAddr, 'delegateToFTSO', [selectedProvider.delegationAddress], provider);
        showNotification(`Successfully delegated to ${selectedProvider.displayName || selectedProvider.delegationAddress}`, 'success');
      } catch (error) {
